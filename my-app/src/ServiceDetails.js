...
const ServiceDetails = () => {
    ...
    fetch(`http://localhost:3001/api/services/${id}`)
      ...
      .then(data => {
        if (!data || data.length === 0) {
          ...
          setService(data);
          if (user && user.uid) {
            fetch(`http://localhost:3001/api/ratings/${id}/user/${user.uid}`) // Usa el ID del usuario autenticado
              .then(response => {
                if (!response.ok) {
  ...
  const statusStyles = service.status === 'Available' ? styles.statusAvailable : styles.statusUnavailable;

  return (
    <div style={styles.container}>
      <div style={styles.card}>
        <div style={styles.cardImage}>
          <img src={service.user_img} alt={service.service_type} style={styles.image} />
        </div>
        <div style={styles.cardBody}>
          <div style={styles.cardHeader}>
            <h1 style={styles.cardTitle}>{service.service_type}</h1>
            <h2 style={styles.cardSubtitle}>Offered by: {service.first_name} {service.last_name}</h2>
            <div style={{ ...styles.status, ...statusStyles }}>{service.status}</div>
            <div style={styles.ratingSection}>
              {renderStars(parseFloat(service.rating) || 0)}
              <p style={styles.ratingScore}>{averageRating} / 5.0 ({service.rating_count} reviews)</p>
            </div>
          </div>
          <div style={styles.detailRow}>
            <span style={styles.detailLabel}>Description:</span>
            <span style={styles.detailValue}>{service.description}</span>
          </div>
          <div style={styles.detailRow}>
          ...
          <div style={styles.detailRow}>
            <span style={styles.detailLabel}>Modalities:</span>
